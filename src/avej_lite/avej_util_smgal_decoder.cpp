...
bool DecodeSMgal_Native(const unsigned char* p_stream, unsigned int stream_size, smgal_decoder::TImage32Bits& image)
{
	...
	// Image Identification Field is not supported
	...
	iu::auto_array<unsigned char> mem_holder(image.p_bitmap);
	...
	case  8:
		{
			...
			//?? temporary, color key assigned automatically
			{
	...
	case 32:
		{
			...
			else
			{
				while (--h >= 0)
				{
					...
					//?? you should consider the endian of current device
...
namespace avej_lite
{
	namespace util
	{
		...
		bool DecodeSMgal(avej_lite::TPixelFormat pixel_format, const char* sz_file_name, avej_lite::TSurfaceDesc& out_surface_desc)
		{
			...
			iu::auto_array<unsigned char> buffer(new unsigned char[file_length]);
